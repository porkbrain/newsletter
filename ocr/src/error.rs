...
impl From<google_vision1::Error> for Error {
    fn from(e: google_vision1::Error) -> Self {
        use google_vision1::Error::*;

        match e {
            UploadSizeLimitExceeded(sent, limit) => Error::new(format!(
                "Upload size limit of {} reached with attempt of {}",
                limit, sent
            )),
            HttpError(_)
            | BadRequest(_)
